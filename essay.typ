...
SSH состоит из трёх главных слоёв, каждый из которых отвечает за разные аспекты безопасной
коммуникации.

== Транспортный уровень
На этом уровне обеспечивается безопасное, зашифрованное подключение между клиентом и сервером,
как было описано выше (см. @connection).

== Уровень пользовательского подключения
Данный уровень отвечает за аутентификацию пользователя, определяя его личность и позволяя
подключение только разрешённым клиентам.

== Уровень подключений
Данный уровень позволяет иметь несколько логических каналов, работающих через единственное
SSH соединение. Каждый из таких каналов может использоваться для разных целей, таких как
доступ к командной строке, передача файлов, портов TCP или передача графической информации
таких технологий как X11 и Wayland. Данная способность протокола SSH позволяет использовать
разные части сервера без необходимости создавать новые соединения на каждую из задач.

= Криптография в SSH
SSH использует много разных аспектов современной криптографии для создания безопасного
подключения: 
+ Симметричное шифрование
  Симметричная криптография используется для шифрования передаваемых данных после установки 
  соединения. Наиболее распространёнными алгоритмами симметричного шифрования являются:
  - AES (Advanced Encription Standard) является стандартом шифрования, принятым правительством США
  - Blowfish - алгоритм шифрования с ключём переменной длинны, разработанный Брюсам Шнайнером в 1993 году. Часто этот алгоритм предоставляет лучшую производительность чем AES
  - 3DES это устаревший алгоритм шифрования, который, тем не менее, всё ещё используется на более старых системах.
+ Ассимитричное шифрование
  Ассиметричная криптография используется для установки соединения и аутентификации пользователя. Наиболее распространёнными алгоритмами явяются:
  - RSA (от фамилий Rivest-Shamir-Adleman) - это широкораспространённый алгоритм, основывающийся на сложности вычисения больших простых чисел.
  - DSA (Алгоритм Цифровой Подписи). Этот алгоритм чаще всего используется для создания цифровых подписей, но также применим к обмену ключами.
  - ECSDA (Алгоритм цифровой подписи на эллиптических кривых) - алгоритм, позволяющий уменьшить размер ключей без потери в безопасности.
+ Хэш-функции
  Хэш-функции используются как во многих алгоритмах шифрования, используемых SSH. Позволяя обеспечить как безопасность передаваемых данных, так и безопасность самого подключения. Наиболее распространёнными хэш-функциями являются:
  - MD4 - один из наиболее устаревших хэш функций. Не рекомендована к использованию.
  - SHA-1 - алгоритм, генерирующий для значения произвольной длины (вплоть до $2^64 - 1$ бит) значения хэша длины 160-бит. Основывается на тех же принципах, что и MD4.
  - SHA-256 - Наиболее современная и надёжная хэш-функция. Именно эта функция является предпочтительной заменой устаревшего SHA-1

= Тунелирование
Как было оговорено выше, протокол SSH позволяет открывать несколько каналов передачи информации через единственное SSH соединение. Перечислим наиболее распространённые применения данной возможности протокола:
+ Перенаправление портов
  SSH-тунель позволяет безопасно перенаправлять порты одного устройства на другие порты второго устройства. Перенаправление портов позволяет подключаться к сервисам одного сервера через устройство в другой, незащищённой сети без необходимости открывать этот порт для доступа из незащищённой сети. Автор данной статьи использует перенаправление портов через SSH для подключения к своим web-сервисам, расположенным на его сервере в другом городе.
+ Перенаправление X11
  SSH также широко используется для перенаправления данных с графического сервера X11 или
  Wayland. Это позволяет многим пользователям со слабыми устройствами подключаться к одному высокопроизводительному серверу для работы над такими сложными графическими приложениям как обработка фото и видео файлов.

= Рекомендации к использованию
Перечислим некоторые рекомендации по использованию протокола SSH на своих устройствах и серверах:
+ Запрет аутентификации по паролю
  Рекомендуется отключить возможность подключения с использованием имени пользователя и пароля. Данные метод подключения считается крайне небезопасный и подлежит атакам.
+ Регулярное обновление ключей
  SSH ключи обеспечивают высокий уровень безопасности, но для наибольшей защищенности даже их необходимо регулярно менять.
+ Использование современных алгоритмов
  Для повышения безопасности подключения рекомендуется использовать наиболее современный хэш-функции (Например SHA-256).
+ Просмотр журнала
  Регулярное наблюдение за журналом вашего сервера поможет заранее заметить подозрительную активность и предотвратить потенциальные атаки.
+ Замена SSH порта
  Рекомендуется сразу заменять порт, используемый вашим устройством для SSH соединений. Значение порта по умолчанию 22 и злоумышленники знают этот факт, концентрируя свои атаки на этот порт.
+ Настройки файрвола
  Для обеспечения дополнительной безопасности можно ограничить набор IP-адресов, которым разрешено подключаться к вашему серверу по SSH.

= Заключение
В современном мире протокол SSH является неотъемлимой частью передачи информации. Сферы его применения варьируются от использования компьютерных игр на удалённых устройствах и рабочих задач, таких как удалённая разработка и компиляция приватного кода на закрытых от внешней сети серверах, до таких требовательных к вычислетельной мощности задач как обработка и создание графического фото и видео контента (В том числе 3D). Многослойная структура этого алгоритма и утилизация им современных криптографических механизмов в группе с настраиваемой аутентификацией делаю SSH поистине незаменимым во всех отраслях технологической сферы. Понимая принцип работы этого протокола и используя рекомендации пользователи могут значительно увеличить безопасность своих подключений в всё более цифровом мире. SSH находится в активной разработке как большими компаниями, так и сообществами добровольцев и адаптируется к новейшим открытиям в мире криптографии. Всё это обеспечивает неустаревающую безопасность.
