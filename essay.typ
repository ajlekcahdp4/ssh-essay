...
= Актуальность
В современном мире компьютеры являются неотъемлимой частью
...
вычислительной мощности многим людям сразу. Также крайне важной задачей
...
давать нескольким людям доступ к одному высокопроизводительному компьютеру
для одновременной удалённой работы. Об этом протоколе далее и пойдёт речь.
...
запрос на специально выделенный TCP (Transfer Control Protocol) порт. Данный порт выбирается
...
позволяющий нескольким
  ...
  и приватного ключа, обмены публичными ключами через незащищённое соединение
...
- Протокол Диффи-Хеллмана на элепьических кривых (ECDH) - вариация протокола
  Диффи-Хеллмана с использование элиптической криптографии. Полагаясь на
  сложность дискретного логарифмирования на эллиптической кривой, данный
  алгоритм позволяет быстрее генерировать ключи меньшей длинны без потери
  защищённости.
...
передаваемых по протоколу. Начиная с этого момента подключение можно считать
защищённым.
...
Использование пароля. В данном методе клиент пересылает имя пользователя и
пароль на
  сервер. Несмотря на простоту, этот способ аутентификации не рекомендуется
  использовать из-за меньшей безопасности.
- Аутентификация при помощи публичного ключа клиента. В данном методе снова
используется
  ассиметричное шифрование. Для его работы сервер хранит публичный ключ клиент
  и проверяет подключающегося клиента на наличее соответсвующего приватного
  ключа.
После успеха аутентификации SSH подключение считается установленным и устройства
могут занятся передачей полезной нагрузки.

= Структура протокола SSH
SSH состоит из трёх главных слоёв, каждый из которых
отвечает за разные аспекты безопасной коммуникации.
...
для создания безопасного подключения: + Симметричное шифрование
  ...
  симметричного шифрования являются: - AES (Advanced Encription Standard) является стандартом
  шифрования, принятым правительством США - Blowfish - алгоритм шифрования с ключём
  переменной длинны, разработанный Брюсам Шнайнером в 1993 году. Часто этот
  алгоритм предоставляет лучшую производительность чем AES - 3DES это устаревший
  алгоритм шифрования, который, тем не менее, всё ещё используется на более
  старых системах.
+ Ассимитричное шифрование
  ...
  явяются: - RSA (от фамилий Rivest-Shamir-Adleman) - это широкораспространённый алгоритм,
  основывающийся на сложности вычисения больших простых чисел.  - DSA (Алгоритм
  Цифровой Подписи). Этот алгоритм чаще всего используется для создания цифровых
  подписей, но также применим к обмену ключами.  - ECSDA (Алгоритм цифровой подписи
  на эллиптических кривых) - алгоритм, позволяющий уменьшить размер ключей без
...
+ Хэш-функции
  ...
  являются: - MD4 - один из наиболее устаревших хэш функций. Не рекомендована
  к использованию.  - SHA-1 - алгоритм, генерирующий для значения произвольной
  длины (вплоть до $2^64 - 1$ бит) значения хэша длины 160-бит. Основывается на тех же
  принципах, что и MD4.  - SHA-256 - Наиболее современная и надёжная хэш-функция. Именно
  эта функция является предпочтительной заменой устаревшего SHA-1
...
возможности протокола: + Перенаправление портов
  ...
  своим web-сервисам, расположенным на его сервере в другом городе.
+ Перенаправление X11
  ...
  сложными графическими приложениям как обработка фото и видео файлов.

= Рекомендации к использованию
Перечислим некоторые рекомендации по
использованию протокола SSH на своих устройствах и серверах: + Запрет
аутентификации по паролю
  Рекомендуется отключить возможность подключения с использованием имени
  пользователя и пароля. Данные метод подключения считается крайне небезопасный
  и подлежит атакам.
...
+ Замена SSH порта
  Рекомендуется сразу заменять порт, используемый вашим устройством для SSH
  соединений. Значение порта по умолчанию 22 и злоумышленники знают этот факт,
  концентрируя свои атаки на этот порт.
+ Настройки файрвола
...
= Заключение В современном мире протокол SSH является неотъемлимой частью
...
таких требовательных к вычислетельной мощности задач как обработка и создание
...
настраиваемой аутентификацией делаю SSH поистине незаменимым во всех отраслях
